...
class Bot::Market
  ...
  def initialize(market_id, account, mode=0x0)
    ...
    @open_orders = Bot::Orderbook::OpenOrders.new(id)
    @orderbook = Bot::Orderbook::Orderbook.new(id)
    apply_flags(mode)
    register_callbacks
  end
  ...
  def add_order(order)
    if order.market.upcase == id.upcase
      Bot::Log.debug { "Order created market:#{id} order:#{order}" }
      @open_orders.add_order(order)
    else
      Bot::Log.debug { "Order skipped market:#{id} order:#{order}" }
    end
  end
