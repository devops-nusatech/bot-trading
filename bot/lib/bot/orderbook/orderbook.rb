...
module Bot::Orderbook
  class Orderbook < Base
      ...
      if limit_bids_base
        ...
        self[:buy].each do |price, amount|
          ...
          if limit_bids_quote && (volume_bids_quote + amount_quote > limit_bids_quote)
            ...
            ::Bot::Log.warn "Bids volume throttled to #{amount} because of quote currency"
            do_break = true
          end
      ...
      if limit_asks_base
        ...
        self[:sell].each do |price, amount|
          ...
          if limit_asks_quote && (volume_asks_quote + amount_quote > limit_asks_quote)
            ...
            amount = amount_quote / price
            ::Bot::Log.warn "Asks volume throttled to #{amount} because of quote currency"
            do_break = true
          end
    ...
    def adjust_volume_simple(adjusted_volume_base, adjusted_volume_quote)
      ...
      self[:buy].each do |price, amount|
        order = Bot::Order.new(@market, price, amount * bids_ratio, :buy)
      ...
      self[:sell].each do |price, amount|
        order = Bot::Order.new(@market, price, amount * asks_ratio, :sell)
