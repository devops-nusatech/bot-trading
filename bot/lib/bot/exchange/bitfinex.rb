...
module Bot::Exchange
  class Bitfinex < Base
    ...
    def subscribe_trades(market, ws)
      ...
      Bot::Log.info "Open event" + sub.to_s
      EM.next_tick {
    ...
    def message_event(msg)
      ...
      # when "auth"
      when "subscribed"
        Bot::Log.info "Event: #{msg}"
      # when "unsubscribed"
      # when "info"
      # when "conf"
      when "error"
        Bot::Log.info "Event: #{msg} ignored"
      else
        # Ignore
      end
    end
    ...
    def new_order(data)
      ...
      Bot::Order.new(@market, price, amount, side)
    end
    ...
    def process_event_message(msg)
      ...
      # when "auth"
      when "subscribed"
        Bot::Log.info "Event: #{msg['event']}"
      # when "unsubscribed"
      # when "info"
      # when "conf"
      when "error"
        Bot::Log.info "Event: #{msg['event']} ignored"
      else
        # Ignore
      end
    end
    ...
    def fetch_openorders(market)
      ...
      orders.select {|o| o["symbol"].upcase == market && o["is_live"] == true }.map do |o|
        order = Bot::Order.new(o["symbol"].upcase, o["price"].to_f, o["remaining_amount"].to_f, o["side"].to_sym)
        order.id = o["id"]
        order
      end
    end
