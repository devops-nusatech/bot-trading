  ...
  class Valr < Base
    ...
    def fetch_openorders(market=nil)
      ...
      authenticated_get("/v1/orders/open").body&.each do |o|
        if market.nil? || market.upcase == o["currencyPair"].upcase
          order = Bot::Order.new(o["currencyPair"].upcase, o["price"].to_f, o["remainingQuantity"].to_f, o["side"].to_sym)
          order.id = o["orderId"]
          orders << order
        end
      end
    ...
    def create_order(order)
      ...
      if response.status >= 300
        Bot::Log.warn "ACCOUNT:#{id} Failed to create order #{order} status:#{response.status}(#{response.reason_phrase}) body:#{response.body}"
      end
