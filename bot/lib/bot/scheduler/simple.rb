    ...
    def schedule
      ...
      %i[buy sell].each do |side|
        ...
        current.each do |_price, orders|
          orders.each do |id, order|
            list.push(::Bot::Action.new(:order_stop, @target, id: id, order: order))
          end
        end
      end
      ...
      %i[buy sell].each do |side|
        ...
        desired.each do |price, amount|
          ...
          amount = apply_precision(amount, @target.amount_precision, @target.min_amount)
          if price.positive? && amount.positive?
            list.push(::Bot::Action.new(:order_create, @target, order: ::Bot::Order.new(@market, price, amount, side)))
          end
        end
      end
