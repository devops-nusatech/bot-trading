  ...
  class Smart < Simple
    ...
    def cancel_risky_orders(side, desired_best_price)
      ...
      @current_ob[side].each do |price, orders|
        orders.each do |_id, order|
          ...
          action = ::Bot::Action.new(:order_stop, @target, order: order, priority: priority)
    ...
    def adjust_levels(side, price_levels, desired_best_price, low_liquidity, high_liquidity)
      ...
      price_levels.each_with_index do |price_point, i|
        raise "PricePoint expected, got #{price_point.class}" unless price_point.is_a?(::Bot::PricePoint)
        ...
        while diff_amount.positive?
          ...
          diff_amount -= amount
          order = ::Bot::Order.new(@market, price, amount, side)
          action = ::Bot::Action.new(:order_create, @target, order: order, priority: priority)
