...
module Bot::Strategy
  ...
  class MicrotradesMarket < Base
    ...
    def get_amount(side)
      ...
      apply_precision(amount, target.amount_precision.to_f, target.min_amount.to_f)
    end
    ...
    def call
      Fiber.new do
        ...
        order = Bot::Order.new(target.id, get_price(side), get_amount(side), side)
