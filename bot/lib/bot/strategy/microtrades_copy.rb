...
module Bot::Strategy
  class MicrotradesCopy < Base
    ...
    def config_markets
      sources.each do |s|
        s.apply_flags(::Bot::Helpers::Flags::LISTEN_PUBLIC_TRADES)
        s.account.add_market_to_listen(s.id)
        cb = proc {|trade| on_trade(s.id, trade) }
        s.account.register_on_public_trade_cb(&cb)
      end
      target.apply_flags(::Bot::Helpers::Flags::LISTEN_PUBLIC_ORDERBOOK)
    ...
    def trigger_microtrade(trade)
      orders_f = Fiber.new do
        ...
        marker = Bot::Order.new(target.id, price, amount, maker_side)
        ...
        taker = Bot::Order.new(target.id, price, amount, taker_side)
