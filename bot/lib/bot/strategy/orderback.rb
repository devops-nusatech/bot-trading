...
module Bot::Strategy
  ...
  class Orderback < Base
    include ::Bot::Helpers::PricePoints
    include ::Bot::Helpers::Spread
    include ::Bot::Helpers::Flags
    ...
    def initialize(sources, target, config, reactor)
      ...
      @plugins = {
        limit_balance: Bot::Plugin::LimitBalance.new(target.account, target.base, target.quote, params)
      }
    ...
    def notify_private_trade_with_trust(trade)
      ...
      order = Bot::Order.new(trade.market, trade.price, trade.volume, trade.type, @orderback_type)
    ...
    def order_back(trade, order)
      ...
      EM::Synchrony.add_timer(@orderback_grace_time) do
        ...
        grouped_trades.each do |k, v|
          order = Bot::Order.new(source.id, k[0].to_f, v, k[1].to_sym, @orderback_type)
        ...
        orders.each do |order|
          actions << Bot::Action.new(:order_create, source, order: order)
        end
    ...
    def call
      ...
      price_points_asks = price_points_asks&.map {|pp| ::Bot::PricePoint.new(apply_spread(:sell, pp.price_point, @spread_asks)) }
      price_points_bids = price_points_bids&.map {|pp| ::Bot::PricePoint.new(apply_spread(:buy, pp.price_point, @spread_bids)) }
