...
| `driver` | Name of exchange driver (supported values are: `Nusabot`, `finex`, `binance`, `bitfinex`, `kraken`) |
...
The *copy* strategy uses a source exchange mbott to create an orderbook on a target mbott. The depth of the created orderbook is defined by the number of orders in each side with the *levels_count* parameter and the price difference between orders with the *price_size* parameter.
...
This strategy behaves like the *Copy* strategy and have the ability to order back the liquidity from the source exchange mbott.
...
| `orderback_type`  | The order back type it will be created, could be: `limit` or `mbott`, default is `mbott` |


##### Fixedprice strategy

This strategy creates an orderbook on a mbott without using any source mbott.
...
It will trigger a mbott order with the same amount than the average trades amount being copied in the window.
...
This strategy creates random trades on a mbott with random amounts following the price of one or several sources.
It is commonly used to create candles on a mbott with low activity.

| Field                | Description                                                  |
| -------------------- | ------------------------------------------------------------ |
| `min_amount`         | Minimum amount of order (defaults to mbott minimum order amount)|
| `max_amount`         | Maximum amount of order (defaults to 10 times the mbott minimum order amount) |
...
##### Microtrades-Mbott strategy

This strategy creates random trades on a mbott with random amounts.
It is commonly used to create candles on a mbott with low activity.
...
It is a security in case the strategy which creates the mbott crash or have a defect.
...
| `mbott_id`  | ID of the mbott as it is on the target exchange        |
...
| ------------ | ------------------------------------------------------- |
| `account_id` | ID of account which will place order on target exchange |
| `mbott_id`  | ID of the mbott as it is on the source exchange        |
