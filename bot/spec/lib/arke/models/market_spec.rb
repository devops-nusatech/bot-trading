...
describe Bot::Market do
  let(:market) { Bot::Market.new(market_id, account, mode) }
  ...
  let(:account) { Bot::Exchange::Base.new(exchange_config) }
  let(:mode) { 0x0 }
  ...
  context "FORCE_MARKET_LOWERCASE flag set" do
    let(:mode) { Bot::Helpers::Flags::FORCE_MARKET_LOWERCASE }
  ...
  context "missing amount_precision from market configuration" do
    ...
    context "WRITE mode" do
      let(:mode) { Bot::Helpers::Flags::WRITE }
      it "raises an error" do
