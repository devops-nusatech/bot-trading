...
describe Bot::ActionExecutor do
  let(:delay) { 0.5 }
  let(:config) do
    {
      "target" => {
  ...
  let(:account) do
    Bot::Exchange::Bitfaker.new(
      "delay" => delay
    )
  end

  let(:target) { Bot::Market.new(config["target"]["market"], account) }
  let(:market) { config["target"]["market"]["id"] }
  let(:sources) {}
  let!(:action_executor) { Bot::ActionExecutor.new(account) }
  let(:actions) { [] }

  context "actions aren't empty, no delay" do
    let(:order_buy) { Bot::Order.new(market, 1, 1, :buy) }
    let(:action1) { Bot::Action.new(:order_create, target, order: order_buy) }
    let(:actions) { [action1] }
    let(:delay) { 0.0 }
  ...
  context "actions aren't empty, delay 0.5 sec" do
    let(:order_buy) { Bot::Order.new(market, 1, 1, :buy) }
    let(:order_sell) { Bot::Order.new(market, 1.1, 1, :sell, "limit", 12) }
    let(:order_sell2) { Bot::Order.new(market, 1.4, 1, :sell) }
    let(:order_buy2) { Bot::Order.new(market, 1.4, 3, :buy, "limit", 42) }
    let(:action1) { Bot::Action.new(:order_create, target, order: order_buy) }
    let(:action2) { Bot::Action.new(:order_stop, target, order:  order_sell) }
    let(:action3) { Bot::Action.new(:order_stop, target, order:  order_buy2) }
    let(:action4) { Bot::Action.new(:order_create, target, order: order_sell2) }
