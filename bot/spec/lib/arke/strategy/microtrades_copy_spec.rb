...
describe Bot::Strategy::MicrotradesCopy do
  let(:reactor) { double(:reactor) }
  let(:strategy) { Bot::Strategy::MicrotradesCopy.new([source], target, config, reactor) }
  ...
  let(:source) { Bot::Market.new(config["sources"].first["market_id"], source_account, Bot::Helpers::Flags::DEFAULT_SOURCE_FLAGS) }
  ...
  context "strategy execution" do
    let(:source) { Bot::Market.new("XBTUSDT", account, source_mode) }
    let(:min_amount) { 10 }
    let(:max_amount) { 10 }
    ...
    let(:public_trade) { ::Bot::PublicTrade.new(42, "XBTUSDT", "kraken", "buy", 0.1, 9999, 999.9) }
    ...
    context "expiration time is reached" do
      it "creates a sell maker order on the target" do
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 9999, 10, :sell, "limit"))
        strategy.instance_variable_set(:@expiration, Time.now.to_i - 10)
      ...
      it "creates a sell order and take it with a buy order on the target" do
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 9999, 10, :sell, "limit")).ordered
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 9999, 10, :buy, "limit")).ordered
        strategy.instance_variable_set(:@expiration, Time.now.to_i - 10)
      ...
      it "cancels orders if they have not been marched before the timeout" do
        ...
        sell = ::Bot::Order.new("BTCUSD", 9999, 10, :sell, "limit", 10)
        buy = ::Bot::Order.new("BTCUSD", 9999, 10, :buy, "limit", 11)
    ...
    context "trade price is higher than the best ask" do
      let(:public_trade) { ::Bot::PublicTrade.new(42, "XBTUSDT", "kraken", "buy", 0.1, 10_015, 1001.5) }
      let(:orderbook) do
        [
          nil,
          [
      ...
      it "creates a sell order and take it with a buy order on the target" do
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 10_009.999999, 10, :sell, "limit")).ordered
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 10_009.999999, 10, :buy, "limit")).ordered
        strategy.instance_variable_set(:@expiration, Time.now.to_i - 10)
    ...
    context "trade price is lower than the best bid" do
      let(:public_trade) { ::Bot::PublicTrade.new(42, "XBTUSDT", "kraken", "buy", 0.1, 9975, 997.5) }
      let(:orderbook) do
        [
          nil,
          [
      ...
      it "creates a sell order and take it with a buy order on the target" do
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 9980.000001, 10, :sell, "limit")).ordered
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 9980.000001, 10, :buy, "limit")).ordered
        strategy.instance_variable_set(:@expiration, Time.now.to_i - 10)
    ...
    context "trade market is not the one configured" do
      let(:public_trade) { ::Bot::PublicTrade.new(42, "XBTETH", "kraken", "buy", 0.1, 139, 13.9) }
    ...
    context "trade market case is different that the one configured" do
      let(:public_trade) { ::Bot::PublicTrade.new(42, "xbtusdt", "kraken", "buy", 0.1, 9985, nil) }
      it "does not create any order" do
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 9985, 10, :sell, "limit")).ordered
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 9985, 10, :buy, "limit")).ordered
        strategy.instance_variable_set(:@expiration, Time.now.to_i - 10)
    ...
    context "strategy with fx" do
      ...
      it "creates a sell order and take it with a buy order on the target" do
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 4999.5, 10, :sell, "limit")).ordered
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 4999.5, 10, :buy, "limit")).ordered
        strategy.instance_variable_set(:@expiration, Time.now.to_i - 10)
