...
describe Bot::Strategy::Copy do
  include_context "mocked binance"

  let!(:strategy) { Bot::Strategy::Copy.new([source], target, config, nil) }
  let(:source_account) { Bot::Exchange.create(source_config) }
  ...
  let(:target) { Bot::Market.new(config["target"]["market_id"], target_account, Bot::Helpers::Flags::DEFAULT_TARGET_FLAGS) }
  let(:side) { "both" }
  ...
  context "binance source" do
    ...
    it "generates a desired orderbook with every level" do
      ...
      Bot::Log.info("buy: #{ob[:buy].to_hash.inspect}")
      Bot::Log.info("sell: #{ob[:sell].to_hash.inspect}")
    end
  end
