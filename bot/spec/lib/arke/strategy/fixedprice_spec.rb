...
describe Bot::Strategy::Fixedprice do
  let(:strategy) { Bot::Strategy::Fixedprice.new([], target, config, nil) }
  let(:account) { Bot::Exchange.create(config["target"]) }
  let(:target) { Bot::Market.new(config["target"]["market_id"], account) }
