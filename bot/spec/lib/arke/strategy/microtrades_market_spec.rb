...
describe Bot::Strategy::MicrotradesMarket do
  let(:reactor) { double(:reactor) }
  let(:strategy) { Bot::Strategy::MicrotradesMarket.new([], target, config, reactor) }
  ...
  context "strategy execution" do
    let(:source) { Bot::Market.new(config["target"]["market_id"], account, source_mode) }
    let(:linked_strategy_id) { 12 }
    let(:linked_strategy) { Bot::Strategy::Base.new([source], target, config, reactor) }
    let(:min_amount) { 250 }
    let(:max_amount) { 250 }
    ...
    context "on bids side" do
      ...
      it "creates a buy order on the target" do
        ...
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 145.803, 250, :buy, "limit"))
    ...
    context "on asks side" do
      ...
      it "creates a buy order on the target" do
        ...
        expect(target.account).to receive(:create_order).with(::Bot::Order.new("BTCUSD", 131.841, 171.339326, :sell, "limit"))
        expect(target.account).to receive(:stop_order).once
  ...
  context "strategy is linked to an other" do
    ...
    it "raise an error if the orderbook is empty" do
      expect(reactor).to receive(:find_strategy).with(linked_strategy_id).exactly(:twice).and_return(linked_strategy)
      expect { strategy.get_price(:buy) }.to raise_error(Bot::Strategy::MicrotradesMarket::EmptyOrderBook)
      expect { strategy.get_price(:sell) }.to raise_error(Bot::Strategy::MicrotradesMarket::EmptyOrderBook)
    end
  end
end
