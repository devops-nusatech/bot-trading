describe Bot::Orderbook::OpenOrders do
  let(:market) { "ethusd" }
  let(:open_orders) { Bot::Orderbook::OpenOrders.new(market) }
  let(:delete_order) { Bot::Order.new(market, 100, 30, :buy, "limit", 1) }
  let(:skip_order) { Bot::Order.new(market, 200, 20, :sell, "limit", 2) }
  let(:create_order) { Bot::Order.new(market, 250, 20, :sell, "limit", 3) }
  let(:update_order) { Bot::Order.new(market, 500, 10, :buy, "limit", 4) }
  let(:update_order_ob) { Bot::Order.new(market, 500, 5, :buy, "limit", 5) }

  let(:orderbook) do
    orderbook = Bot::Orderbook::Orderbook.new(market)
  ...
  context "#get_by_id" do
    it "returns the order with the given id" do
      orders = Bot::Orderbook::OpenOrders.new(market)
      order1 = Bot::Order.new(market, 100, 1, :sell, "limit", 1)
      order2 = Bot::Order.new(market, 100, 2, :sell, "limit", 2)
      order3 = Bot::Order.new(market, 102, 3, :sell, "limit", 3)
      order4 = Bot::Order.new(market, 103, 4, :sell, "limit", 4)
  ...
  context "#total_side_amount" do
    ...
    context "several orders for the same price" do
      it "calculates the total amount in orderbook side" do
        orders = Bot::Orderbook::OpenOrders.new(market)
        orders.add_order(Bot::Order.new(market, 100, 1, :sell, "limit", 1))
        orders.add_order(Bot::Order.new(market, 100, 2, :sell, "limit", 2))
        orders.add_order(Bot::Order.new(market, 100, 3, :sell, "limit", 3))
        orders.add_order(Bot::Order.new(market, 100, 4, :sell, "limit", 4))
  ...
  context "group_by_level" do
    let(:price_points_sell) { [6, 8].map {|price| ::Bot::PricePoint.new(price) } }

    let(:order_sell_0)     { Bot::Order.new("ethusd", 5, 1, :sell, "limit", 0) }
    let(:order_sell_1)     { Bot::Order.new("ethusd", 8, 2, :sell, "limit", 1) }
    let(:order_sell_2)     { Bot::Order.new("ethusd", 2, 3, :sell, "limit", 2) }
