...
describe ::Bot::Orderbook::Orderbook do
  let(:market)     { "ethusd" }
  let(:orderbook)  { ::Bot::Orderbook::Orderbook.new(market) }

  context "orderbook#add" do
    let(:order_buy)   { ::Bot::Order.new(market, 1, 1, :buy) }
    let(:order_sell)  { ::Bot::Order.new(market, 1, 1, :sell) }
    let(:order_sell2) { ::Bot::Order.new(market, 1, 1, :sell) }
  ...
  context "orderbook#remove" do
    ...
    it "removes correct order from orderbook" do
      orderbook.update(order_buy)
      orderbook.update(::Bot::Order.new(market, order_buy.price, 1, :buy))
      orderbook.update(::Bot::Order.new(market, 11, 1, :sell))
    ...
    it "does nothing if non existing id" do
      ...
      orderbook.delete(::Bot::Order.new(market, 10, 1, :buy))
  ...
  context "orderbook#merge" do
    let(:ob1) { ::Bot::Orderbook::Orderbook.new(market) }
    let(:ob2) { ::Bot::Orderbook::Orderbook.new(market) }
    let(:ob3) { ::Bot::Orderbook::Orderbook.new(market) }

    it "merges two orderbooks into one" do
      ob1.update(::Bot::Order.new(market, 10, 10, :sell))
      ob1.update(::Bot::Order.new(market, 20, 15, :sell))
      ob1.update(::Bot::Order.new(market, 25, 5, :sell))

      ob2.update(::Bot::Order.new(market, 10, 30, :sell))
      ob2.update(::Bot::Order.new(market, 20, 20, :sell))
      ob2.update(::Bot::Order.new(market, 10, 10, :buy))

      ob3.update(::Bot::Order.new(market, 10, 40, :sell))
      ob3.update(::Bot::Order.new(market, 20, 35, :sell))
      ob3.update(::Bot::Order.new(market, 25, 5, :sell))
      ob3.update(::Bot::Order.new(market, 10, 10, :buy))
  ...
  context "adjust_volume" do
    let(:price_points_buy) { [::Bot::PricePoint.new(8), ::Bot::PricePoint.new(6)] }
    let(:price_points_sell) { [::Bot::PricePoint.new(6), ::Bot::PricePoint.new(8)] }
    let(:order_sell_0)     { ::Bot::Order.new("ethusd", 5, 1, :sell) }
    let(:order_sell_1)     { ::Bot::Order.new("ethusd", 8, 1, :sell) }
    let(:order_sell_2)     { ::Bot::Order.new("ethusd", 2, 1, :sell) }
    let(:order_buy_0)      { ::Bot::Order.new("ethusd", 5, 1, :buy) }
    let(:order_buy_1)      { ::Bot::Order.new("ethusd", 8, 1, :buy) }
    let(:order_buy_2)      { ::Bot::Order.new("ethusd", 9, 1, :buy) }
  ...
  context "adjust_volume_simple" do
    let(:ob) { ::Bot::Orderbook::Orderbook.new(market) }

    before(:each) do
      ob.update(::Bot::Order.new("ethusd", 5, 4, :sell))
      ob.update(::Bot::Order.new("ethusd", 8, 4, :sell))
      ob.update(::Bot::Order.new("ethusd", 2, 4, :sell))

      ob.update(::Bot::Order.new("ethusd", 5, 3, :buy))
      ob.update(::Bot::Order.new("ethusd", 8, 3, :buy))
      ob.update(::Bot::Order.new("ethusd", 9, 3, :buy))
    end
  ...
  context "adjust_volume_simple complex amounts" do
    let(:ob) { ::Bot::Orderbook::Orderbook.new(market) }
    ...
    before(:each) do
      ...
      ob.update(::Bot::Order.new("ethusd", 5, 5, :buy))
      ob.update(::Bot::Order.new("ethusd", 8, 3, :buy))
      ob.update(::Bot::Order.new("ethusd", 9, 1, :buy))
    end
    ...
    it "adjusts volume according to requirements" do
      ...
      expect(book[:buy].to_hash).to eq(
        5.0.to_d => "2.84482758620689655".to_d,
        8.0.to_d => "1.70689655172413793".to_d,
        9.0.to_d => "0.56896551724137931".to_d
      )
      expect(book.volume_asks_base).to eq(6)
      expect(book.volume_asks_quote).to eq(30)
      expect(book.volume_bids_base).to eq("5.12068965517241379".to_d)
      expect(book.volume_bids_quote).to be_within(0.00000001).of(33)
    end
  end
