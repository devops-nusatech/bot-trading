...
describe Bot::Orderbook::Aggregated do
  let(:market)     { "ethusd" }
  let(:orderbook)  { Bot::Orderbook::Orderbook.new(market) }
  let(:order_sell_0)     { Bot::Order.new("ethusd", 5, 2, :sell) }
  ...
  let(:price_points_buy) do
    [8, 6].map {|price| ::Bot::PricePoint.new(price) }
  end
  let(:price_points_sell) do
    [6, 8].map {|price| ::Bot::PricePoint.new(price) }
  end
  ...
  context "aggregate orderbook with ranges without orders" do
    ...
    let(:orderbook) { Bot::Orderbook::Orderbook.new(market, sell: sell_side, buy: buy_side) }
    let(:price_points_sell) do
      [1000, 1001, 1002, 1003, 1004].map {|price| ::Bot::PricePoint.new(price) }
    end
    let(:price_points_buy) do
      [1021, 1020, 1019, 1018, 1017].map {|price| ::Bot::PricePoint.new(price) }
    end
  ...
  context "aggregate orderbook with price points over the ranges" do
    ...
    let(:orderbook) { Bot::Orderbook::Orderbook.new(market, sell: sell_side, buy: buy_side) }
    let(:price_points_sell) do
      [1000, 1001, 1005, 1010].map {|price| ::Bot::PricePoint.new(price) }
    end
    let(:price_points_buy) do
      [999, 990, 980].map {|price| ::Bot::PricePoint.new(price) }
    end
  ...
  context "aggregate big orderbook" do
    ...
    let(:orderbook) { Bot::Orderbook::Orderbook.new(market, sell: sell_side) }
    let(:price_points_sell) do
      [12_593.30, 12_593.40, 12_593.50, 12_593.60, 12_594.00, 12_594.50, 12_595, 12_596, 12_597, 12_600]
        .map {|price| ::Bot::PricePoint.new(price) }
    end
