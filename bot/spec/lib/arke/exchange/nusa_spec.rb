...
describe Bot::Exchange::Nusabot do
  ...
  let(:market) { Bot::Market.new(market_id, Nusabot) }
  let(:order) { Bot::Order.new("ethusd", 1, 1, :buy) }
  let(:Nusabot) { Bot::Exchange::Nusabot.new(exchange_config) }

  context "mocked Nusabot" do
    include_context "mocked Nusabot"
    before(:each) { order.apply_requirements(Nusabot) }

    context "Nusabot#create_order" do
      it "gets 403 on request with wrong api key" do
        Nusabot.instance_variable_set(:@api_key, SecureRandom.hex)

        expect(Nusabot.create_order(order).id).to eq nil
      end

      it "doesn't updates open_orders after create" do
        ord = Nusabot.create_order(order)
    ...
    context "Nusabot#fetch_openorders" do
    ...
    context "Nusabot#get_balances" do
      it "get balances of all users accounts" do
        expect(Nusabot.get_balances).to eq(
          [
    ...
    context "Nusabot#stop_order" do
      let(:order) { Bot::Order.new("ethusd", 1, 1, :buy, "limit", 42) }
      ...
      it "cancels an open order and doesn't notify when the cancel is pending" do
        ...
        Nusabot.register_on_deleted_order(&cb.method(:call))
    ...
    context "Nusabot#process_message" do
      let(:order) { Bot::Order.new("ethusd", 1, 1, :buy) }
      let(:updated_order) { Bot::Order.new("ETHUSD", 1.0, 0.5, :buy) }
      ...
      let(:order_id) { Nusabot.create_order(order).id }
      ...
      before do
        Nusabot.create_order(order).id
      end

      it "updates order when partially fullfilled" do
        market
        Nusabot.send(:ws_read_message, :private, order_partially_fullfilled)
      ...
      it "removes order when cancelled" do
        Nusabot.send(:ws_read_message, :private, order_cancelled)
        expect(market.open_orders[:buy].length).to eq(0)
      end

      it "sends a callback on executed trade" do
        expect(Nusabot).to receive(:notify_private_trade).twice
        Nusabot.send(:ws_read_message, :private, trade_executed)
      end
    end

    context "Nusabot#cancel_all_orders" do
      let(:order) { Bot::Order.new("ethusd", 1, 1, :buy, "limit", 12) }
      let(:order_second) { Bot::Order.new("ethusd", 1, 1, :sell, "limit", 13) }
  ...
  context "Nusabot#market_config after peatio 2.2.14" do
    include_context "mocked Nusabot"
    it "generates market configuration" do
      expect(Nusabot.market_config("btcusd")).to eq(
        "id"               => "btcusd",
        "base_unit"        => "btc",
        "quote_unit"       => "usd",
        "min_price"        => 100.0,
  ...
  context "Nusabot#market_config before peatio 2.2.14" do
    it "generates market configuration" do
  ...
  context "Nusabot#market_config misconfiguration" do
    ...
    it "raises error if id is missing" do
      ...
      expect { Nusabot.market_config("btcusd") }.to raise_error("Market btcusd not found")
    end
    ...
    it "raises error if base_unit is missing" do
      ...
      expect { Nusabot.market_config("btcusd") }.to raise_error(/base_unit/)
    end
    ...
    it "raises error if quote_unit is missing" do
      ...
      expect { Nusabot.market_config("btcusd") }.to raise_error(/quote_unit/)
    end
    ...
    it "raises error if amount_precision is missing" do
      ...
      expect { Nusabot.market_config("btcusd") }.to raise_error(/amount_precision/)
    end
    ...
    it "raises error if price_precision is missing" do
      ...
      expect { Nusabot.market_config("btcusd") }.to raise_error(/price_precision/)
    end
  end
  ...
  context "finex enabled" do
    ...
    let(:order) { Bot::Order.new("ethusd", 1, 2, :buy) }
    before(:each) { order.apply_requirements(Nusabot) }
    ...
    it "cancels an open order and doesn't notify when the cancel is pending" do
      o = Bot::Order.new("ethusd", 1, 1, :buy, "limit", 42)
      cb = double("on_deleted_order callback", call: true)
      Nusabot.register_on_deleted_order(&cb.method(:call))
      expect(cb).to_not receive(:call)
      market.stop_order(o)
  ...
  context "maintain market public orderbook incrementally" do
    ...
    it "updates an existing price point" do
      Nusabot.send(:ws_read_private_message, snapshot)
      Nusabot.send(:ws_read_private_message, "ethusd.ob-inc" => {"asks" => ["252.32", "0.1"], "sequence" => 6112})
      expect(Nusabot.update_orderbook("ethusd")[:buy].to_hash).to eq(
        249.16.to_d => 0.20603.to_d,
        248.69.to_d => 0.09944.to_d,
        248.66.to_d => 0.05057.to_d
      )
      expect(Nusabot.update_orderbook("ethusd")[:sell].to_hash).to eq(
    ...
    it "supports bulk events updates" do
      Nusabot.send(:ws_read_private_message, snapshot)
      Nusabot.send(:ws_read_private_message, "ethusd.ob-inc" => {"asks" => [["252.32", "0.1"], ["252.33", "0.2"], ["252.92", ""]], "sequence" => 6112})
      expect(Nusabot.update_orderbook("ethusd")[:buy].to_hash).to eq(
        249.16.to_d => 0.20603.to_d,
        248.69.to_d => 0.09944.to_d,
        248.66.to_d => 0.05057.to_d
    ...
    it "deletes an existing price point" do
      Nusabot.send(:ws_read_private_message, snapshot)
      Nusabot.send(:ws_read_private_message, "ethusd.ob-inc" => {"asks" => ["252.32", "0.0"], "sequence" => 6112})
      Nusabot.send(:ws_read_private_message, "ethusd.ob-inc" => {"bids" => ["248.69", "0.0"], "sequence" => 6113})
      expect(Nusabot.update_orderbook("ethusd")[:buy].to_hash).to eq(
        249.16.to_d => 0.20603.to_d,
        248.66.to_d => 0.05057.to_d
      )
    ...
    it "deletes an existing price point (again)" do
      Nusabot.send(:ws_read_private_message, snapshot)
      Nusabot.send(:ws_read_private_message, "ethusd.ob-inc" => {"asks" => ["252.32", ""], "sequence" => 6112})
      Nusabot.send(:ws_read_private_message, "ethusd.ob-inc" => {"bids" => ["248.69", ""], "sequence" => 6113})
      expect(Nusabot.update_orderbook("ethusd")[:buy].to_hash).to eq(
        249.16.to_d => 0.20603.to_d,
        248.66.to_d => 0.05057.to_d
      )
    ...
    it "disconnects websocket if it detects a sequence out of order" do
      ...
      Nusabot.instance_variable_set(:@ws, ws)
      expect(ws).to receive(:close)
      Nusabot.send(:ws_read_private_message, "ethusd.ob-inc" => {"asks" => ["252.32", ""], "sequence" => 6113})
    end
  end
  ...
  context "update balances on ranger messages" do
    ...
    it "updates an existing price point" do
      Nusabot.send(:ws_read_private_message, balances_event)
      expect(Nusabot.fetch_balances).to eq(
        [
          {
            "currency" => "btc",
            "free"     => 0.998,
      ...
      expect(Nusabot.balance("btc")).to eq(
        "currency" => "btc",
        "free"     => 0.998,
        "locked"   => 0.002,
        "total"    => 1,
      )
      expect(Nusabot.balance("eth")).to eq(
        "currency" => "eth",
        "free"     => 1_000_000_000,
        "locked"   => 0,
        "total"    => 1_000_000_000,
      )
      expect(Nusabot.balance("trst")).to be_nil
    end
  end
end
