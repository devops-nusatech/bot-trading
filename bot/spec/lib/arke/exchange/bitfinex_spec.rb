...
describe Bot::Exchange::Bitfinex do
  ...
  let(:strategy) { Bot::Strategy::Copy.new(config) }
  let(:bitfinex) { Bot::Exchange::Bitfinex.new(bitfinex_config) }
  let!(:market) { Bot::Market.new(market_id, bitfinex) }
  ...
  context "fetch_openorders" do
    it "fetchs the account openorders and stores them into local openorder cache" do
      ...
      expect(market.open_orders[:buy][0.02]).to eq(123 => Bot::Order.new("ETHUSD", 0.02, 0.2, :buy))
    end
  end

  context "create order" do
    let(:order) { Bot::Order.new("ETHUSD", 135.84, 6.62227, :buy) }
  ...
  context "public trade event" do
    let(:trade) do
      Bot::PublicTrade.new(418_288_902, "ETHUSD", "bitfinex", "sell", 0.1, 262.69, 26.269, 1_582_036_305_000)
    end
    let(:trade_event) do
      OpenStruct.new(
        "type": "message",
