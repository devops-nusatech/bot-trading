...
describe Bot::Exchange::Huobi do
  include_context "mocked huobi"

  before(:all) { Bot::Log.define }

  let(:faraday_adapter) { :em_synchrony }
  let(:huobi) do
    Bot::Exchange::Huobi.new(
      "host"           => "api.huobi.pro",
  ...
  let!(:market) { Bot::Market.new(market_id, huobi) }

  context "ojbect initialization" do
    it "is a sublass of Bot::Exchange::Base" do
      expect(Bot::Exchange::Huobi.superclass).to eq(Bot::Exchange::Base)
    end

    it "has an orderbook" do
      expect(market.orderbook).to be_an_instance_of(Bot::Orderbook::Orderbook)
    end
  end

  context "getting snapshot" do
    let(:snapshot_buy_order_1) { Bot::Order.new("ethusdt", 135.84000000, 6.62227000, :buy) }
    let(:snapshot_buy_order_2) { Bot::Order.new("ethusdt", 135.85000000, 0.57176000, :buy) }
    let(:snapshot_buy_order_3) { Bot::Order.new("ethusdt", 135.87000000, 36.43875000, :buy) }

    let(:snapshot_sell_order_1) { Bot::Order.new("ethusdt", 135.91000000, 0.00070000, :sell) }
    let(:snapshot_sell_order_2) { Bot::Order.new("ethusdt", 135.93000000, 8.00000000, :sell) }
    let(:snapshot_sell_order_3) { Bot::Order.new("ethusdt", 135.95000000, 1.11699000, :sell) }
  ...
  context "fetch_openorders" do
    it "fetchs the account openorders and stores them into local openorder cache" do
      ...
      expect(market.open_orders[:buy][0.452]).to eq(43 => Bot::Order.new("ethusdt", 0.452, 0.3, :buy))
      expect(market.open_orders[:sell][0.453]).to eq(42 => Bot::Order.new("ethusdt", 0.453, 1.0, :sell))
    end
  end

  context "order_create" do
    let(:order) { Bot::Order.new("ethusdt", 250, 1, :buy) }
  ...
  context "public trade event received on websocket" do
    ...
    it "notifies public trade to registered callbacks" do
      ...
      expect(callback).to receive(:call).with(Bot::PublicTrade.new(101_902_657_304, "ethusdt", "huobi", :sell, 5.5303.to_d, 272.86.to_d, 5.5303.to_d * 272.86.to_d, 1_582_449_913_472))
      expect(callback).to receive(:call).with(Bot::PublicTrade.new(101_902_657_303, "ethusdt", "huobi", :sell, 0.1947.to_d, 272.86.to_d, 0.1947.to_d * 272.86.to_d, 1_582_449_913_472))
      expect(callback).to receive(:call).with(Bot::PublicTrade.new(101_902_657_302, "ethusdt", "huobi", :sell, 0.0192.to_d, 272.88.to_d, 0.0192.to_d * 272.88.to_d, 1_582_449_913_472))
      expect(callback).to receive(:call).with(Bot::PublicTrade.new(101_902_657_301, "ethusdt", "huobi", :sell, 2.0.to_d, 272.89.to_d, 2.0.to_d * 272.89.to_d, 1_582_449_913_472))
      expect(callback).to receive(:call).with(Bot::PublicTrade.new(101_902_657_300, "ethusdt", "huobi", :sell, 4.275.to_d, 272.89.to_d, 4.275.to_d * 272.89.to_d, 1_582_449_913_472))
      expect(callback).to receive(:call).with(Bot::PublicTrade.new(101_902_657_299, "ethusdt", "huobi", :sell, 1.9808.to_d, 272.89.to_d, 1.9808.to_d * 272.89.to_d, 1_582_449_913_472))
