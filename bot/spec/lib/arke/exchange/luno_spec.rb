...
describe Bot::Exchange::Luno do
  include_context "mocked luno"
  let(:luno) do
    Bot::Exchange::Luno.new(
      "key"    => "abcdefghijklm",
      "secret" => "skhfksjhgksdjhfksjdfkjsdfksjhdkfsj"
    )
  end
  let(:market_id) { "XBTZAR" }
  let!(:market) { Bot::Market.new(market_id, luno) }
  ...
  context "update_orderbook" do
    let(:snapshot_buy_order_1) { Bot::Order.new("XBTZAR", 1100.00, 0.10, :buy) }
    let(:snapshot_buy_order_2) { Bot::Order.new("XBTZAR", 1000.00, 0.10, :buy) }
    let(:snapshot_buy_order_3) { Bot::Order.new("XBTZAR", 900.00, 0.10, :buy) }

    let(:snapshot_sell_order_1) { Bot::Order.new("XBTZAR", 1180.00, 0.10, :sell) }
    let(:snapshot_sell_order_2) { Bot::Order.new("XBTZAR", 2000.00, 0.10, :sell) }
  ...
  context "create_order" do
    let(:order) { Bot::Order.new("XBTZAR", 1180.00, 0.10, :sell) }
