...
describe Bot::Exchange::Hitbtc do
  include_context "mocked hitbtc"
  let(:hitbtc) do
    Bot::Exchange::Hitbtc.new(
      "host"   => "api.hitbtc.com",
      "key"    => "abcdefghijklm",
  ...
  let!(:market) { Bot::Market.new(market_id, hitbtc) }
  ...
  context "update_orderbook" do
    let(:snapshot_buy_order_1) { Bot::Order.new("ETHUSD", 0.046001, 0.005, :buy) }
    let(:snapshot_buy_order_2) { Bot::Order.new("ETHUSD", 0.046, 0.2, :buy) }

    let(:snapshot_sell_order_1) { Bot::Order.new("ETHUSD", 0.046002, 0.088, :sell) }
    let(:snapshot_sell_order_2) { Bot::Order.new("ETHUSD", 0.046800, 0.2, :sell) }
  ...
  context "create_order" do
    let(:order) { Bot::Order.new("ETHUSD", 1180.00, 0.10, :sell) }
