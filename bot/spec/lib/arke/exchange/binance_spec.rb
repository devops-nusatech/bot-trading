...
describe Bot::Exchange::Binance do
  ...
  context "getting snapshot" do
    let(:snapshot_buy_order_1) { Bot::Order.new("ETHUSDT", 2, 1, :buy) }
    let(:snapshot_sell_order_1) { Bot::Order.new("ETHUSDT", 6, 1, :sell) }
  ...
  context "fetch_openorders" do
    it "fetchs the account openorders and stores them into local openorder cache" do
      ...
      expect(market.open_orders[:buy][0.1]).to eq(42 => Bot::Order.new("LTCBTC", 0.1, 0.9, :buy))
    end
  end

  context "order_create" do
    let(:order) { Bot::Order.new("ETHUSDT", 250, 1, :buy) }
    let(:small_order) { Bot::Order.new("ETHUSDT", 5, 1, :buy) }
    let(:very_small_order) { Bot::Order.new("ETHUSDT", 1, 1, :buy) }
  ...
  context "get_amount" do
    let(:order) { Bot::Order.new("ETHUSDT", 250, 1, :buy) }
    let(:danger_order) { Bot::Order.new("ETHUSDT", 3, 3, :buy) }
    let(:small_order) { Bot::Order.new("ETHUSDT", 1, 1, :buy) }
    ...
    it "amount of market order without price" do
      o = Bot::Order.new("ETHUSDT", nil, 1, :buy)
  ...
  context "receive public trades events on websocket" do
    ...
    let(:trade) do
      Bot::PublicTrade.new(6_026_899, "ENGBTC", "binance", "sell", 1242.0, 0.000037, 0.45954e-1, 1_571_640_883_296)
    end

    let(:aggTrade) do
      Bot::PublicTrade.new(173_732_494, "BTCUSDT", "binance", "buy", 0.060037, 7974.32, 0.47875424984e3, 1_571_514_775_994)
    end
  ...
  context "receive public depth events on websocket" do
    ...
    let(:binance) do
      b = Bot::Exchange::Binance.new({})
      b.apply_flags(Bot::Helpers::Flags::LISTEN_PUBLIC_ORDERBOOK)
    ...
    it "orderbook size will be WS_ORDERBOOK_MIN_CACHE_SIZE if current size exeeced WS_ORDERBOOK_MAX_CACHE_SIZE and return with sorted prices" do
      stub_const("Bot::Exchange::Binance::WS_ORDERBOOK_MIN_CACHE_SIZE", 1)
      stub_const("Bot::Exchange::Binance::WS_ORDERBOOK_MAX_CACHE_SIZE", 2)
      binance.send(:ws_read_public_message, depthUpdate_event_ethusdt)
