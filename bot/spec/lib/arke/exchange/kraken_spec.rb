...
describe Bot::Exchange::Kraken do
  ...
  let(:kraken) { Bot::Exchange::Kraken.new(exchange_config) }
  let(:market_id) { "XBTUSD" }

  let(:market) { Bot::Market.new(market_id, kraken) }
  ...
  context "private endpoints" do
    ...
    context "fetch_openorders" do
      it "returns the list of open orders" do
        expect(kraken.fetch_openorders("XRPUSD")).to eq(
          [
            ::Bot::Order.new("XRPUSD", 0.22, 100, :sell, :limit, "O7FHIE-CPMCK-65GSWL")
          ]
        )
      end
    end
    ...
    context "create_order" do
      it "creates an order" do
        order = ::Bot::Order.new("XRPUSD", 0.22, 100, :sell, :limit)
    ...
    context "stop_order" do
      it "cancels an order" do
        order = ::Bot::Order.new(nil, nil, nil, nil, nil, "O7FHIE-CPMCK-65GSWL")
    ...
    context "public trade event received on websocket" do
      ...
      it "notifies public trade to registered callbacks" do
        ...
        expect(callback).to receive(:call).with(Bot::PublicTrade.new("1582445053.916545", "ethusd", "kraken", :sell, 0.03490924.to_d, 271.69.to_d, 9.4844914156.to_d, 1_582_445_053_916_545.to_d / 1e6))
        expect(callback).to receive(:call).with(Bot::PublicTrade.new("1582445053.923872", "ethusd", "kraken", :sell, 0.00009052.to_d, 271.69.to_d, 0.0245933788.to_d, 1_582_445_053_923_872.to_d / 1e6))
        expect(callback).to receive(:call).with(Bot::PublicTrade.new("1582445053.925416", "ethusd", "kraken", :sell, 0.00000024.to_d, 271.69.to_d, 0.0000652056.to_d, 1_582_445_053_925_416.to_d / 1e6))
